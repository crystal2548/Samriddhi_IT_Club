...
const FORM_FIELDS: [string, keyof SponsorForm, string][] = [
  ['Name', 'name', ''],
  ['Logo URL', 'logo_url', 'Cloudinary URL'],
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(0,212,255,0.2)', borderRadius: 14, padding: 24, marginBottom: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            ...
            <div>
              ...
              <select value={form.tier} onChange={e => setForm(p => ({ ...p, tier: e.target.value as SponsorTier }))} style={{ ...IS, cursor: 'pointer' }}>
                {(['gold', 'silver', 'bronze', 'partner'] as SponsorTier[]).map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
