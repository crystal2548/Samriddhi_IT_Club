    ...
    return (
      <div style={{ textAlign: 'center', padding: '100px 20px' }}>
        ...
        <p style={{ color: 'var(--text-muted)', maxWidth: 400, margin: '0 auto' }}>
          Only club executives and OC members can post new opportunities.
        </p>
      </div>
    )
  }
      ...
      {showForm && (
        <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(0,212,255,0.2)', borderRadius: 14, padding: 24, marginBottom: 24 }}>
          <h3 style={{ color: '#fff', fontSize: 15, fontWeight: 600, marginBottom: 20 }}>
            {editId ? 'Edit Opportunity' : 'Share New Opportunity'}
          </h3>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            <FormField label="Link *" value={form.link} onChange={v => setForm(p => ({ ...p, link: v }))} placeholder="https://..." />
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              ...
              <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} rows={3} style={TS} placeholder="Briefly describe the opportunity..." />
            </div>

            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <input type="checkbox" id="is_active" checked={form.is_active} onChange={e => setForm(p => ({ ...p, is_active: e.target.checked }))} />
              <label htmlFor="is_active" style={{ color: 'var(--text-secondary)', fontSize: 13, cursor: 'pointer' }}>Active (Visible to public)</label>
            </div>
          </div>
