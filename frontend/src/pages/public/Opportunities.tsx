    ...
    <div style={{ background: 'var(--bg-primary)', minHeight: '100vh', paddingTop: 80, paddingBottom: 100 }}>
      <Container>
        ...
        <div style={{ marginBottom: 48 }}>
          <Link
            to="/resources"
            style={{ display: 'inline-flex', alignItems: 'center', gap: 8, color: 'var(--text-muted)', fontSize: 13, textDecoration: 'none', marginBottom: 24, transition: 'color 0.2s' }}
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: 15, maxWidth: 600, margin: 0 }}>
          ...
          {types.map(t => (
            <button
              ...
              style={{
                padding: '8px 18px',
                borderRadius: 8,
                background: filter === t ? 'var(--pink)' : 'rgba(255,255,255,0.03)',
        ...
        {loading ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 24 }}>
            {[1, 2, 3, 4].map(i => (
              <div key={i} style={{ height: 120, background: 'rgba(255,255,255,0.02)', borderRadius: 12, border: '1px solid var(--border)' }} />
            ))}
          </div>
        ) : filtered.length > 0 ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 24 }}>
            {filtered.map(o => (
              <a href={o.link} target="_blank" rel="noopener noreferrer" key={o.id} style={{ textDecoration: 'none' }}>
                <div
                  ...
                  onMouseEnter={e => {
                    e.currentTarget.style.borderColor = 'rgba(255,45,155,0.3)'
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                    <span style={{
                      ...
                      background: 'rgba(255,255,255,0.04)', color: 'var(--pink)', textTransform: 'uppercase',
                    }}>
                      {o.type}
                    </span>
                    {o.deadline && (
                      <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                        Ends {formatDateShort(o.deadline)}
                      </span>
                    )}
                  </div>

                  <h3 style={{ color: '#fff', fontSize: 18, fontWeight: 700, marginBottom: 10, margin: '0 0 10px' }}>
                    {o.title}
                  </h3>
                  <p style={{ color: 'var(--text-secondary)', fontSize: 13, lineHeight: 1.6, marginBottom: 20, margin: '0 0 20px' }}>
                    {o.description}
                  </p>
